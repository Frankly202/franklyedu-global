...
export const Route = createFileRoute("/marketplace")({
  head: () => pageMeta("Student Marketplace", "Accommodation, real estate, essentials and services for students settling in."),
    ...
    <>
      ...
      <Section>
        <div className="mb-10 grid gap-4 md:grid-cols-2">
          <Link to="/accommodation" className="card-navy group p-6 transition-transform hover:-translate-y-0.5">
            <p className="eyebrow text-sky-soft">Housing</p>
            <h2 className="mt-2 text-2xl font-bold">Accommodation</h2>
            <p className="mt-2 text-sm text-navy-muted">Dormitories, shared flats and private apartments near campus.</p>
            <span className="mt-4 inline-block text-sm font-semibold">Browse accommodation →</span>
          </Link>
          <Link to="/real-estate" className="card-navy group p-6 transition-transform hover:-translate-y-0.5">
            <p className="eyebrow text-sky-soft">Property</p>
            <h2 className="mt-2 text-2xl font-bold">Real Estate</h2>
            <p className="mt-2 text-sm text-navy-muted">Buy or rent long-term — including student investment properties.</p>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((m) => (
            <article key={m.slug} className="card-light flex flex-col p-5">
              <span className="self-start rounded-full bg-muted px-2.5 py-0.5 text-[0.65rem] font-semibold">{m.category}</span>
              ...
              <div className="mt-auto flex items-center justify-between pt-5">
                ...
                <a href={whatsappLink(`Hi, I'm interested in "${m.title}" on the marketplace.`)} target="_blank" rel="noreferrer" className="btn btn-primary btn-sm">Enquire</a>
              </div>
            </article>
          ))}
        </div>
