...
export const Route = createFileRoute("/student")({
  head: () => pageMeta("Student Dashboard", "Track your applications, saved universities and document checklist."),
      ...
      <section className="bg-navy-deep py-10 text-navy-foreground">
        <div className="container-site grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4">
          <div className="min-w-0">
            ...
            <h1 className="mt-1 truncate text-2xl font-bold sm:text-3xl">Hello, {s.name.split(" ")[0]}</h1>
            <p className="text-sm text-navy-muted">Target intake: {s.targetIntake} · {s.nationality}</p>
          </div>
          <Link to="/application" className="btn btn-primary btn-sm shrink-0">New application</Link>
        </div>
      </section>
          ...
          <div className="space-y-6">
                ...
                {s.applications.map((a) => (
                  <article key={a.id} className="card-light p-5">
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      ...
                      <span className="rounded-full bg-navy px-2.5 py-1 text-[0.65rem] font-semibold text-navy-foreground">{a.status}</span>
                    </div>
                    <div className="mt-4 h-1.5 rounded-full bg-muted">
                      <div className="h-full rounded-full bg-sky" style={{ width: `${a.progress}%` }} />
                    </div>
            ...
            <div>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                {saved.map((u) => (
                  <Link key={u.slug} to="/courses" search={{ university: u.slug }} className="card-navy p-4 transition-transform hover:-translate-y-0.5">
          ...
          <aside className="space-y-6">
            <div className="card-light p-5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-muted-foreground">{doneCount}/{s.checklist.length}</span>
              </div>
              <ul className="mt-3 space-y-2 text-sm">
                {s.checklist.map((c) => (
                  <li key={c.item} className="flex items-center gap-2">
                    <span className={`grid h-4 w-4 place-items-center rounded-full text-[0.6rem] ${c.done ? "bg-sky text-sky-foreground" : "border border-border"}`}>{c.done ? "✓" : ""}</span>
            ...
            <div className="card-light p-5 text-sm">
              ...
              <Link to="/login" className="mt-3 inline-block text-xs font-semibold hover:underline">Sign out (prototype) →</Link>
            </div>
          </aside>
        </div>
      </Section>
