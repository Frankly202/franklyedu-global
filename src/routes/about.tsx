        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {aboutContent.stats.map((s) => (
            <div key={s.label} className="card-navy p-6 text-center">
              ...
              <p className="mt-1 text-xs text-navy-muted">{s.label}</p>
            </div>
          ))}
        </div>
      </Section>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {aboutContent.values.map((v) => (
            <div key={v.title} className="card-navy p-6">
              ...
              <p className="mt-2 text-sm text-navy-muted">{v.text}</p>
            </div>
          ))}
        </div>
      </Section>
      <Section>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {aboutContent.team.map((t) => (
            <div key={t.name} className="card-light flex items-center gap-4 p-5">
